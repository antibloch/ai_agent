...
def logger_node(state: AgentState) -> Dict[str, Any]:
    msgs = list(state.get("messages", []))
    cursor = int(state.get("history_cursor", 0))

    print("\n\n============================================================")
    print("[TRACE] New messages")
    print("------------------------------------------------------------")
    ...
    return {"history_cursor": len(msgs)}
...
async def build_graph():
    ...
    # log after assistant
    workflow.add_edge("assistant", "logger")

    # decision should be based on assistant output
    ...
    workflow.add_edge("logger", "assistant")
...
async def main():
    ...
    inputs = {"messages": [HumanMessage(content=query)], "history_cursor": 0}
